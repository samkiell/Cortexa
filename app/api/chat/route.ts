...
export async function POST(req: Request) {
  try {
    ...
    const settings = await Settings.findOne();
    const apiKeyRaw = settings?.openrouterApiKey || settings?.veniceApiKey || settings?.featherlessApiKey || process.env.OPENROUTER_API_KEY || process.env.VENICE_API_KEY || process.env.FEATHERLESS_API_KEY;
