...
export default function SettingsForm({ initialData, initialModels = [] }: SettingsFormProps) {
  const [apiKey, setApiKey] = useState(initialData?.veniceApiKey || initialData?.openrouterApiKey || initialData?.featherlessApiKey || '');
    ...
    const fetchData = async () => {
      try {
        ...
        if (settingsRes.ok) {
          ...
          setApiKey(settings.veniceApiKey || settings.openrouterApiKey || settings.featherlessApiKey || '');
